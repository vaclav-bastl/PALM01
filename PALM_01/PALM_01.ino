...
inline uint8_t getSelectorCc(uint8_t p) {
  return presetSelectorCC[p];
}
...
static inline bool detectDominant7Pair(bool leftHandLocal,
  ...
  auto a_b = [&](uint8_t a, uint8_t b) -> uint8_t {
    return leftHandLocal ? a : b;
  };
  ...
  if (tC && tB) {
    seventhOut = true;
    rootOut = MIDI_NOTE_C3;
    g_rootTouchIdxPrimary = idxB;
    g_rootTouchIdxSecondary = rngA;
    return true;
  }
  if (tD && tC) {
    seventhOut = true;
    rootOut = MIDI_NOTE_D3;
    g_rootTouchIdxPrimary = midB;
    g_rootTouchIdxSecondary = idxB;
    return true;
  }
  if (tE && tD) {
    seventhOut = true;
    rootOut = MIDI_NOTE_E3;
    g_rootTouchIdxPrimary = rngB;
    g_rootTouchIdxSecondary = midB;
    return true;
  }
  if (tF && tE) {
    seventhOut = true;
    rootOut = MIDI_NOTE_F3;
    g_rootTouchIdxPrimary = litB;
    g_rootTouchIdxSecondary = rngB;
    return true;
  }
  if (tG && tF) {
    seventhOut = true;
    rootOut = MIDI_NOTE_G3;
    g_rootTouchIdxPrimary = idxA;
    g_rootTouchIdxSecondary = litB;
    return true;
  }
  if (tA && tG) {
    seventhOut = true;
    rootOut = MIDI_NOTE_A3;
    g_rootTouchIdxPrimary = midA;
    g_rootTouchIdxSecondary = idxA;
    return true;
  }
  if (tB && tA) {
    seventhOut = true;
    rootOut = MIDI_NOTE_B3;
    g_rootTouchIdxPrimary = rngA;
    g_rootTouchIdxSecondary = midA;
    return true;
  }
...
int getRootFromTouchesLocal(bool leftHandLocal) {
  ...
  if (detectDominant7Pair(leftHandLocal, sev, rootIfPair)) {
    g_addSeventh = sev;
    return rootIfPair;  // g_rootTouchIdxPrimary already set inside detectDominant7Pair()
  }
  ...
  g_rootTouchIdxPrimary = (int8_t)minIdx;  // remember which touch produced the root
  return minMidi;
}
...
inline void midiNoteOnHelper(uint8_t ch, uint8_t note, uint8_t vel) {
  sendNoteOn(note, vel, ch);
}
inline void midiNoteOffHelper(uint8_t ch, uint8_t note) {
  sendNoteOff(note, 0, ch);
}

void noteModeAllNotesOffLocal(uint8_t ch) {
  if (lastMainNote >= 0) {
    midiNoteOffHelper(ch, (uint8_t)lastMainNote);
    lastMainNote = -1;
  }
  if (last7thNote >= 0) {
    midiNoteOffHelper(ch, (uint8_t)last7thNote);
    last7thNote = -1;
  }
  noteQuantIdx = -1;
}
...
void handleContext() {
  ...
  if (selectorHeld()) {
    ...
    if (!ccDisabled(selCC)) {
      ...
      const uint8_t values[6] = { 0, 25, 51, 76, 102, 127 };  // evenly spaced 0..127
      int lastIdx = -1;
      for (int i = 0; i < 6; ++i) {
